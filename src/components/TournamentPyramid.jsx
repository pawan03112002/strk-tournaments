...
import { OrbitControls, Text, MeshDistortMaterial, Sparkles } from '@react-three/drei'
import { motion, AnimatePresence } from 'framer-motion'
import { Search, Trophy, Users, Crown, Target, Medal, ChevronRight, Award, Filter } from 'lucide-react'
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      {filteredPlayers.slice(0, 8).map((player, index) => (
        <motion.div
          ...
          className={`p-3 rounded-lg border-2 ${getStatusColor(player.status)} backdrop-blur-sm`}
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white font-bold text-sm">{player.uid}</p>
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-gaming-dark to-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-0 right-0 w-96 h-96 bg-yellow-500 rounded-full blur-3xl animate-pulse-slow"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <h2 className="text-5xl font-bold mb-4 bg-gradient-to-r from-yellow-500 to-orange-500 bg-clip-text text-transparent">
            Tournament Bracket
          </h2>
          <p className="text-gray-400 text-lg">
            Interactive 3D Pyramid - Click layers to explore each stage
          </p>
        </motion.div>
